---
name: design-review
description: Use BEFORE declaring any code change complete. Reviews the change against SOLID, DRY, KISS, SoC, YAGNI, cohesion/coupling, fail-fast, explicitness, single source of truth, and the SHOULD heuristics. Required for all executable-code deliverables across both tiers (React frontend in apps/web and FastAPI backend in apps/api) and shared contracts in generated OpenAPI client. NOT for non-code outputs (docs, content, JQL, SQL reads, plain explanations).
harness:
  tier: shared
  family: process
