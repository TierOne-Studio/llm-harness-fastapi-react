...
3. Citation site (depending on type):
   ...
   | Meta-rule (conflict resolution, asks-first, attribution) | The relevant skill (`decision-rules` § 6, `fastapi-best-practices` prelude, `git-workflow` Hard rules) |
...
> ✅ (backend) "Per `ADR-00X`, throw FastAPI built-in exceptions; no global filter."
