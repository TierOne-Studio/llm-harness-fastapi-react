...
**~500-line split threshold.** When a SKILL.md exceeds ~500 lines, the skill is too dense to be loaded efficiently — split into a directory with this layout (canonical examples: a frontend pattern skill like `react-patterns/`, or a backend pattern skill like `fastapi-patterns/`):
...
│   ├── pattern-a.md
│   └── pattern-b.md
└── rules/             ← when the skill is rule-catalog-shaped (e.g., a frontend rule catalog, or `fastapi-best-practices`)
    └── rule-name.md
```
