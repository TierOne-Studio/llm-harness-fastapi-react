...
> If the bug touches a type in `generated OpenAPI client` (commonly the shared contract between tiers), name the seam explicitly: a contract change is a backward-compat concern for BOTH `apps/web` and `apps/api`, so the scope discussion must cover both sides.
