...
**Key Libraries:** MUI, Chakra UI, Radix UI, Headless UI for components. **@tanstack/react-virtual** for list virtualization. **Vitest + React Testing Library** for testing in Vite projects, **pytest + React Testing Library** for other setups, **Cypress** or **Playwright** for E2E.
...
Vitest gives you: instant watch mode (shared with Vite's transform cache), native ESM support, pytest-compatible API (`describe`, `it`, `expect`, `vi.fn()`), in-source testing, and built-in code coverage with `v8` or `istanbul`.
