...
- A **cross-tier feature** carries **one spec per layer** — a `ui` spec and a `contract` spec —
  ...
  the shared contract in `generated OpenAPI client`. One spec per (feature × layer) — never two for the
...
7. **Architect review** — when the plan touches 3+ files OR auth/RBAC/data-migration/transaction
   boundaries/OpenAPI-contract shape, route the SPEC through `architect-reviewer`
...
1. **Reconcile** — *Affected areas* matches the real diff across `apps/web` + `apps/api` + `generated OpenAPI client` (incl. the migration); ACs updated if behavior changed; each AC linked to its now-green test (file:line).
...
- Shared contract (`generated OpenAPI client`): types/DTOs changed ...
...
Which layer proves which AC — frontend unit/component (Vitest + Testing Library), backend unit/integration (pytest, real Postgres where data/RBAC/migration-bound), seam e2e (Playwright in `e2e/`). Name the test files.
