...
- **Errors are actionable** (typed, contextual, redacted). Use the framework's error/exception types per `repo-conventions`, not plain `Error` — on the backend that means FastAPI built-in exceptions; on the frontend that means typed errors surfaced through the UI (error boundary, toast, form error state).
- **Backward compatibility preserved** unless the user explicitly told you otherwise. For changes to shared contracts (commonly `generated OpenAPI client`), a breaking type change is a backward-compat concern for BOTH tiers — confirm both the producer and the consumer are updated.
