...
# guidance (React frontend + FastAPI backend + shared-contract seam), AND stays free of
...
assert_true "T3: title names BOTH frameworks (FastAPI + React)" \
  "grep -qiE '^# .*FastAPI' '$INSTRUCTIONS' && grep -qiE '^# .*React' '$INSTRUCTIONS'"
...
assert_true "T3: P0 keeps BOTH deploy gate AND DB-write gate (fullstack)" \
  "grep -qiE 'npm publish|PyPI|production rollout' '$INSTRUCTIONS' && grep -qiE 'db-write-protocol|DB writes' '$INSTRUCTIONS'"
...
assert_true "T5: repo-conventions documents the shared-contract seam (generated OpenAPI client)" \
  "grep -qiE 'generated OpenAPI client|OpenAPI contract' '$RC'"
...
# Backend
NCA="$SKILLS/fastapi-clean-architecture/SKILL.md"
...
assert_true "T6(BE): fastapi-patterns covers Depends/Security + APIRouter + lifespan" \
  "grep -qiE 'Depends|Security' '$SKILLS/fastapi-patterns/SKILL.md' && grep -qiE 'APIRouter|lifespan' '$SKILLS/fastapi-patterns/SKILL.md'"
assert_true "T6(BE): fastapi-best-practices covers routers/dependencies/OpenAPI" \
  "grep -qiE 'APIRouter|Dependencies|Depends|OpenAPI' '$SKILLS/fastapi-best-practices/SKILL.md'"
assert_true "T6(BE): OpenAPI contracts skill covers generated client drift" \
  "grep -qiE 'generated TypeScript client|Generated client freshness|Operation IDs' '$SKILLS/openapi-contracts/SKILL.md'"
assert_true "T6(BE): FastAPI testing skill covers pytest dependency overrides" \
  "grep -qiE 'pytest|dependency overrides' '$SKILLS/fastapi-testing/SKILL.md'"
...
assert_true "T7: code-reviewer references BOTH a react-* skill AND a fastapi-* skill" \
  "grep -qE 'react-' '$CR' && grep -qE 'fastapi-' '$CR'"
assert_true "T7: lessons-curator domain map lists BOTH stacks" \
  "grep -qiE 'react-' '$AGENTS/lessons-curator.md' && grep -qiE 'fastapi-' '$AGENTS/lessons-curator.md'"
...
for s in tdd-workflow design-review plan-mode repo-conventions react-patterns \
         react-state-management react-routing frontend-security fastapi-best-practices \
...
assert_true "T12: fastapi-best-practices ships no compiled AGENTS.md" \
  "! test -f '$SKILLS/fastapi-best-practices/AGENTS.md'"
assert_true "T12: fastapi-best-practices ships no build scripts/" \
  "! test -d '$SKILLS/fastapi-best-practices/scripts'"
...
# demand (the fastapi-patterns layout). Warn > 400 lines, fail > 800.
...
# (`fastapi-patterns/patterns/…`, `testing-patterns/…`) from false-matching.
BROKEN_LINKS=0
